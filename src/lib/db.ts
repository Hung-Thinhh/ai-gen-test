...
const MAX_GALLERY_SIZE = 50; // Maximum number of images allowed in gallery

/**
 * Gets the current count of images in the gallery
 */
const getGalleryCount = async (): Promise<number> => {
    const db = await initDB();
...
/**
 * Deletes the oldest images to make room for new ones
 */
const deleteOldestImages = async (count: number): Promise<void> => {
    if (count <= 0) return;
    const db = await initDB();
    const tx = db.transaction(GALLERY_STORE, 'readwrite');
    const store = tx.objectStore(GALLERY_STORE);
    const cursorReq = store.openCursor(); // Opens in ascending order (oldest first)
    let deleted = 0;
    return new Promise((resolve) => {
        cursorReq.onsuccess = () => {
...
export const addGalleryImage = async (imageUrl: string): Promise<void> => {
    const currentCount = await getGalleryCount();
    if (currentCount >= MAX_GALLERY_SIZE) {
        await deleteOldestImages(1);
    }
    const db = await initDB();
    const tx = db.transaction(GALLERY_STORE, 'readwrite');
    const store = tx.objectStore(GALLERY_STORE);
...
export const addMultipleGalleryImages = async (imageUrls: string[]): Promise<void> => {
    const currentCount = await getGalleryCount();
    const newTotal = currentCount + imageUrls.length;

    // If adding new images would exceed limit, delete oldest ones first
    if (newTotal > MAX_GALLERY_SIZE) {
        const toDelete = newTotal - MAX_GALLERY_SIZE;
        await deleteOldestImages(toDelete);
    }
...
/**
 * Gets the latest N images from gallery (newest first)
 */
export const getLatestGalleryImages = async (count: number): Promise<string[]> => {
    const db = await initDB();
    const tx = db.transaction(GALLERY_STORE, 'readonly');
    const store = tx.objectStore(GALLERY_STORE);
    const allItems = store.getAll();
    return new Promise((resolve) => {
        tx.oncomplete = () => {
            // Reverse to get newest first, then take only the first N
            const reversed = (allItems.result as string[]).reverse();
            resolve(reversed.slice(0, count));
        };
    });
};
