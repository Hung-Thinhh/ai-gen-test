...
type StylePresetKey = string; // Dynamic now, not fixed union
...
interface PosterCreatorProps {
    ...
    stylePresets?: Record<string, any>; // Optional: load from DB or use hardcoded
    domainContext?: string; // Optional: domain context from DB
}
...
`;
            ...
            // Pass domainPrompt as the last argument
            // mainPrompt = `${environmentContext}${preset.buildPrompt(desc, posterTypePrompt, bgPrompt, lightPrompt, anglePrompt, notes, domainPrompt)}`;

            // MANUAL PROMPT ASSEMBLY (Code-based)
            const styleDescription = preset.prompt || preset.buildPrompt?.() || ''; // Raw style text
                        ...
                        {!isLoading && (showOptions || !hasResults) && (
                            <motion.div className="w-full max-w-4xl" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }}>
                                <div className="themed-card backdrop-blur-md rounded-2xl p-4 md:p-6">
                                    ...
                                    {/* Domain Selector - HIDDEN: now loaded from DB automatically
                                    <div className="mb-4">
                                        <SearchableSelect
                                            id="domain"
