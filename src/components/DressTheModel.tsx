...
const DressTheModel: React.FC<DressTheModelProps> = (props) => {
    ...
    const Uploader = ({ id, onUpload, caption, description, currentImage, onImageChange, placeholderType, cardType }: any) => (
        <div className="flex flex-col items-center gap-2">
            <label htmlFor={id} className="cursor-pointer group transform hover:scale-105 transition-transform duration-300 w-full">
            ...
            {description && <p className="base-font font-bold text-neutral-300 text-center max-w-xs text-xs sm:text-sm">{description}</p>}
        </div>
    );
        ...
        <div className="flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
            ...
            {appState.stage === 'idle' && (
                <div className="w-full pb-4 max-w-4xl">
                    <motion.div
                        className="grid grid-cols-2 md:grid-cols-4 gap-2 w-full max-w-7xl mx-auto px-4"
