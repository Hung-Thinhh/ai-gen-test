...
export const downloadImage = async (url: string, filenameWithoutExtension: string) => {
    ...
    try {
        ...
        if (url.startsWith('data:')) {
            ...
            if (url.startsWith('data:image/png')) extension = 'png';
        ...
        // Fallback for direct link if fetch fails (e.g. strict CORS)
        const link = document.createElement('a');
        link.href = url;
...
export const dataURLtoBlob = async (dataurl: string): Promise<Blob> => {
    try {
        ...
        // Fallback for older browsers or if fetch fails on data URL (unlikely)
...
export const processAndDownloadAll = async ({
    ...
    historicalImages.forEach((item, index) => {
        ...
        // Check if it's a video by fetching the blob type if it is a blob url
        let isVideo = false;
        let extension = undefined;

        if (url.startsWith('blob:')) {
            // We need to check the mime type. Since we can't do async inside forEach easily (without Promise.all mapping),
            // we will handle this by pushing a promise or handling it in the next step.
            // BUT, wait, processAndDownloadAll creates allItemsToZip which is then passed to downloadAllImagesAsZip.
            // downloadAllImagesAsZip handles blobs correctly!
            // The issue is that we are explicitly setting `extension: 'mp4'` here which overrides detection.

            // We should NOT assume it is a video just because it is a blob.
            // We can leave extension undefined and let downloadAllImagesAsZip figure it out from the blob type.
            // However, downloadAllImagesAsZip uses dataURLtoBlob which might re-fetch.

            // Let's explicitly NOT set extension for blob URLs unless we know it' video tasks.
            // The video tasks loop below handles video explicitly.
            // So here, for historicalImages, we assume they are images unless proven otherwise.
            // But if we have mixed content?
            // "historicalImages" in PosterCreator are usually images.
        }
        ...
        allItemsToZip.push({
            ...
            folder: 'output',
            // extension: isVideo ? 'mp4' : undefined, // REMOVED THIS LINE
        });
...
export const embedJsonInPng = async (imageDataUrl: string, jsonData: object, enabled: boolean): Promise<string> => {
    ...
    try {
        ...
        const chunkType = new TextEncoder().encode('Duky');
