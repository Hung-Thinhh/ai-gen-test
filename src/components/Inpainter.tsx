...
import { inpaintImage } from '../services/gemini/advancedImageService';
import ActionablePolaroidCard from './ActionablePolaroidCard';
import { AppScreenHeader, handleFileUpload as utilHandleFileUpload, ResultsView, OptionsPanel, useAppControls, type InpainterState } from './uiUtils';

interface InpainterProps {
    mainTitle: string;
    subtitle: string;
    ...
    appState: InpainterState;
    onStateChange: (newState: InpainterState) => void;
    onReset: () => void;
    onGoBack: () => void;
...
const Inpainter: React.FC<InpainterProps> = (props) => {
    ...
    const handleImageUpload = (e: ChangeEvent<HTMLInputElement>) => {
        utilHandleFileUpload(e, (imageDataUrl) => {
            onStateChange({ ...appState, maskedImage: imageDataUrl, resultImage: null, error: null });
            // addImagesToGallery([imageDataUrl]);
        });
    };

    const handleGenerate = async () => {
        if (!appState.maskedImage || !appState.options.prompt.trim()) return;
        ...
        try {
            const result = await inpaintImage(appState.options.prompt, appState.maskedImage, appState.options.additionalInstructions || undefined, 'inpainter');
            onStateChange({ ...appState, stage: 'results', resultImage: result });
            addImagesToGallery([result]);
            logGeneration('inpainter', preGenState, result, {
    ...
    const canGenerate = appState.maskedImage && appState.options.prompt.trim();
    ...
    return (
        <div className="flex flex-col items-center justify-center w-full h-full flex-1 min-h-screen">
            ...
            {appState.stage === 'configuring' && (
                <motion.div className="flex flex-col items-center gap-6 w-full max-w-screen-xl py-6 overflow-y-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <div className="w-full pb-4 max-w-4xl">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 w-full max-w-7xl mx-auto px-4">
                            <div className="flex flex-col items-center gap-2">
                                <label htmlFor="inp-upload" className="cursor-pointer w-full"><ActionablePolaroidCard type={appState.maskedImage ? 'multi-input' : 'uploader'} caption={uploaderCaption} status="done" mediaUrl={appState.maskedImage || undefined} placeholderType="magic" onImageChange={(url) => onStateChange({ ...appState, maskedImage: url })} /></label>
                                <input id="inp-upload" type="file" className="hidden" accept="image/*" onChange={handleImageUpload} />
                    ...
                    <OptionsPanel>
                        ...
                        <div><label className="block text-neutral-200 font-bold mb-2">{t('inpainter_prompt')} *</label><input type="text" value={appState.options.prompt} onChange={(e) => onStateChange({ ...appState, options: { ...appState.options, prompt: e.target.value } })} placeholder={t('inpainter_promptPlaceholder')} className="form-input" /></div>
                        <div><label className="block text-neutral-200 font-bold mb-2">{t('common_additionalInstructions')}</label><textarea value={appState.options.additionalInstructions} onChange={(e) => onStateChange({ ...appState, options: { ...appState.options, additionalInstructions: e.target.value } })} placeholder={t('inpainter_instructionsPlaceholder')} className="form-input !h-24" rows={3} /></div>
                ...
                <motion.div className="flex flex-col items-center gap-8 w-full max-w-screen-xl py-6 overflow-y-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <div className="flex flex-col md:flex-row items-start justify-center gap-8 w-full px-4">
                        <div className="flex flex-col items-center gap-4">
                            <ActionablePolaroidCard type="multi-input" caption={uploaderCaption} status="done" mediaUrl={appState.maskedImage!} placeholderType="magic" onImageChange={() => { }} />
                            <p className="text-neutral-300 text-center max-w-xs text-sm">{uploaderDescription}</p>
                        </div>
                        <div className="flex flex-col items-center gap-4">
                            <ActionablePolaroidCard type="output" caption={t('inpainter_result')} status="pending" mediaUrl={undefined} placeholderType="magic" onImageChange={() => { }} />
            ...
            {appState.stage === 'results' && (
                <ResultsView stage={appState.stage} originalImage={appState.maskedImage} onOriginalClick={() => { }} error={appState.error} isMobile={false} actions={(<><button onClick={() => onStateChange({ ...appState, stage: 'configuring' })} className="btn btn-secondary">{t('common_edit')}</button><button onClick={onReset} className="btn btn-secondary">{t('common_startOver')}</button></>)}>
                    {appState.resultImage && (<motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><ActionablePolaroidCard type="output" caption={t('inpainter_result')} status="done" mediaUrl={appState.resultImage} onImageChange={(url) => { if (url) { onStateChange({ ...appState, resultImage: url }); addImagesToGallery([url]); } }} /></motion.div>)}
                </ResultsView>
            )}
        </div>
    );
