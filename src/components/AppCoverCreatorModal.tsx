...
const AppCoverCreatorModal: React.FC<AppCoverCreatorModalProps> = ({ isOpen, onClose }) => {
    ...
    const isVideo = (url: string | null): boolean => url?.startsWith('blob:') ?? false;
    ...
    const handleCombine = async () => {
        ...
        try {
            ...
            downloadImage(resultUrl, `Duky-cover-${Date.now()}`);
        } catch (err) {
    ...
    const handleDragStart = (e: React.DragEvent<HTMLDivElement>, url: string) => { e.dataTransfer.setData('application/Duky-image-url', url); e.dataTransfer.effectAllowed = 'copy'; };
    const handleOutputDragOver = (e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); if (e.dataTransfer.types.includes('application/Duky-image-url')) setIsDraggingOverOutput(true); };
    const handleOutputDrop = (e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); setIsDraggingOverOutput(false); const url = e.dataTransfer.getData('application/Duky-image-url'); if (url && !isVideo(url)) { if (inputImages.includes(url)) setInputImages(prev => prev.filter(img => img !== url)); setOutputImage(url); } };
    const handleInputsDragOver = (e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); if (e.dataTransfer.types.includes('application/Duky-image-url')) setIsDraggingOverInputs(true); };
    const handleInputsDrop = (e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); setIsDraggingOverInputs(false); const url = e.dataTransfer.getData('application/Duky-image-url'); if (url && !isVideo(url) && inputImages.length < 4 && !inputImages.includes(url)) { if (url === outputImage) setOutputImage(null); setInputImages(prev => [...prev, url]); } };
