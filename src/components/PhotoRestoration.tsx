...
const PhotoRestoration: React.FC<PhotoRestorationProps> = (props) => {
    ...
    const executeInitialGeneration = async () => {
        ...
        try {
            const resultUrl = await restoreOldPhoto(appState.uploadedImage, appState.options, 'photo-restoration');
                    ...
                    >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 w-full max-w-7xl mx-auto px-4">
                                <div className="flex flex-col items-center gap-2">
                                    <ActionablePolaroidCard
                                        type={appState.uploadedImage ? 'content-input' : 'uploader'}
                                        mediaUrl={appState.uploadedImage ?? undefined}
                                        caption={uploaderCaption}
                                        placeholderType="person"
                                        status="done"
                                        onClick={appState.uploadedImage ? () => openLightbox(0) : undefined}
                        ...
                        <OptionsPanel>
                            ...
                            <div className="flex items-center justify-end gap-4 pt-4">
                                ...
                                <button onClick={executeInitialGeneration} className="btn btn-primary" disabled={isLoading || !appState.uploadedImage}>{isLoading ? t('photoRestoration_creating') : t('photoRestoration_createButton')}</button>
                            </div>
                        </OptionsPanel>
                    </motion.div>
                )}
