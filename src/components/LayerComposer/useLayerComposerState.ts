...
export const useLayerComposerState = ({ isOpen, onClose, onHide }: { isOpen: boolean; onClose: () => void; onHide: () => void; }) => {
    ...
    const handleExportSelectedLayers = useCallback(async () => {
        ...
        try {
            for (const layer of selectedLayers) {
                ...
                await new Promise(resolve => setTimeout(resolve, 200)); downloadImage(exportedUrl, `Duky-canvas-export-${layer.id || 'layer'}`);
            }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            downloadJson(canvasState, `Duky-canvas-state-${Date.now()}.json`);
            if (!isInfiniteCanvas) {
    ...
    const handlePresetFile = async (file: File) => {
        ...
        try {
            ...
            if (settingsData && settingsData.viewId && settingsData.state) {
                const appConfig = settings?.apps.find((app: any) => app.id === settingsData.viewId);
