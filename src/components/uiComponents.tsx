...
// --- Helper Functions ---
/**
 * Limits the historical images array to prevent memory leaks.
 * Keeps only the most recent N images.
 */
export const limitHistoricalImages = <T,>(existingImages: T[], newImages: T[], maxCount: number = MAX_HISTORICAL_IMAGES): T[] => {
...
export const AppScreenHeader: React.FC<AppScreenHeaderProps> = ({ mainTitle, subtitle, useSmartTitleWrapping, smartTitleWrapWords, titleClassName }) => (
    <motion.div
...
export const ResultsView: React.FC<ResultsViewProps> = ({ stage, originalImage, onOriginalClick, inputImages, children, actions, isMobile, error, hasPartialError }) => {
    ...
    return (
        <div className="w-full flex-1 flex flex-col items-center justify-between pt-12">
            ...
            <div className="w-full flex-1 flex flex-col items-center justify-start overflow-y-auto py-4 results-scroll-container gap-8">
                {/* Input images - compact grid on mobile, 4 cols on desktop */}
                {finalInputImages.length > 0 && (
                    <motion.div
                        layout
                        className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 px-4 md:px-8 w-full max-w-screen-2xl mx-auto"
                    >
