    ...
    return (
        <div className="flex flex-col items-center justify-center w-full h-full flex-1 min-h-screen">
            ...
            <div className="flex flex-col items-center justify-center w-full flex-1 px-4">
                {(appState.stage === 'idle' || appState.stage === 'configuring') && (
                    <motion.div className="flex flex-col items-center gap-6 w-full max-w-6xl py-6 overflow-y-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                        {/* Input images grid */}
                        ...
                        {!appState.styleReferenceImage ? (
                            <div className="w-full max-w-4xl text-center mt-4">
                                ...
                                <div className="mb-4">
                                    <button onClick={handleRandomGenerateClick} className="btn btn-primary btn-sm" disabled={isLoading || isAnalyzing || !appState.uploadedImage}> {t('beautyCreator_randomButton')} </button>
                                </div>
                                <div className="max-h-[50vh] overflow-y-auto p-4 bg-black/20 border border-white/10 rounded-lg space-y-6">
                                    {Array.isArray(IDEAS_BY_CATEGORY) && IDEAS_BY_CATEGORY.map((categoryObj: any) => (
                                        <div key={categoryObj.category}>
                                            <h3 className="text-xl base-font font-bold text-orange-400 text-left mb-3 sticky top-0 bg-black/50 py-2 -mx-4 px-4 z-10">{categoryObj.category}</h3>
                                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                                                {categoryObj.ideas.map((p: string) => {
                                                    const isSelected = appState.selectedIdeas.includes(p);
                                                    return (<button key={p} onClick={() => handleIdeaSelect(p)} className={`base-font font-bold p-2 rounded-sm text-sm transition-all duration-200 ${isSelected ? 'bg-orange-400 text-black ring-2 ring-orange-300 scale-105' : 'bg-white/10 text-neutral-300 hover:bg-white/20'} ${!isSelected && appState.selectedIdeas.length === maxIdeas ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!isSelected && appState.selectedIdeas.length === maxIdeas}> {p} </button>);
                                                })}
                                            </div>
                                        </div>
                                    ))}
                        ...
                        ) : (<div className="w-full max-w-4xl text-center p-4 bg-neutral-700/50 rounded-lg my-4"> <p className="text-sm text-o-300">{t('common_styleReferenceActive')}</p> </div>)}
                        ...
                        <OptionsPanel>
                            ...
                            <div className="flex items-center pt-2">
                                <input type="checkbox" id="remove-watermark-beauty" checked={appState.options.removeWatermark} onChange={(e) => handleOptionChange('removeWatermark', e.target.checked)} className="h-4 w-4 rounded border-neutral-500 bg-neutral-700 text-o-400 focus:ring-o-400 focus:ring-offset-neutral-800" aria-label={t('common_removeWatermark')} />
                            ...
                            <div className="flex items-center justify-end gap-4 pt-4">
                                ...
                                <button onClick={handleGenerateClick} className="btn btn-primary" disabled={isLoading || !appState.uploadedImage || (!appState.styleReferenceImage && appState.selectedIdeas.length === 0)}> {getButtonText()} </button>
                            </div>
                        </OptionsPanel>
                    </motion.div>
                )}
            ...
            {(appState.stage === 'generating' || appState.stage === 'results') && (
                <ResultsView stage={appState.stage} inputImages={inputImagesForResults} error={appState.error} hasPartialError={hasPartialError} actions={<> {appState.historicalImages.length > 0 && !appState.error && (<button onClick={handleDownloadAll} className="btn btn-secondary"> {t('common_downloadAll')} </button>)} <button onClick={handleBackToOptions} className="btn btn-secondary"> {t('common_editOptions')} </button> <button onClick={onReset} className="btn btn-secondary"> {t('common_startOver')} </button> </>} >
                    ...
                    {appState.historicalImages.map(({ url: sourceUrl }) => {
                        ...
                        return (<motion.div className="w-full md:w-auto flex-shrink-0" key={`${sourceUrl}-video`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: 'spring', stiffness: 100, damping: 20 }}>
